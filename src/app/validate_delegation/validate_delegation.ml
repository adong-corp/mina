(* validate_delegations.ml *)

(* check whether a block producer follows requirements
   at https://docs.minaprotocol.com/en/advanced/foundation-delegation-program
*)

(* this code is based on the replayer. Differences:
   - we don't output a target ledger
   - no option to continue after an error
   - we check that a block producer has fulfilled its obligations
*)
...
type delegation_source = O1 | Foundation [@@deriving show]
...
let main ~input_file ~archive_uri ~payout_address () =
  ...
  | Ok pool ->
      ...
      *)
      ...
      let delegation_source =
        if
          List.mem Payout_addresses.foundation_addresses payout_address
            ~equal:String.equal
        then Foundation
        else if
          List.mem Payout_addresses.o1_addresses payout_address
            ~equal:String.equal
        then O1
        else
          failwithf
            ...
            payout_address ()
      in
      [%log info] "Payout address has delegation source $delegation_source"
        ~metadata:
          [ ( "delegation_source"
            , `String (show_delegation_source delegation_source) ) ] ;
      let payout_public_key =
        Public_key.Compressed.of_base58_check_exn payout_address
      in
      let delegatee =
        match
          Ledger.fold_until ledger ~init:None
            ~f:(fun _ (account : Account.t) ->
              let open Continue_or_stop in
              if
                Public_key.Compressed.equal account.public_key
                  payout_public_key
                && Token_id.equal account.token_id Token_id.default
              then
                match account.delegate with
                | Some _ ->
                    Stop account.delegate
                | None ->
                    failwithf
                      "Account for payout address %s in genesis ledger has no \
                       delegate"
                      payout_address ()
              else Continue None )
            ~finish:Fn.id
        ...
        | Some pk ->
            pk
        | None ->
            failwithf "Payout address %s does not appear in genesis ledger"
              payout_address ()
      in
      let delegatee_address =
        Public_key.Compressed.to_base58_check delegatee
      in
      [%log info] "The payout address delegates to public key $delegatee"
        ~metadata:[("delegatee", `String delegatee_address)] ;
      let%bind payment_id = pk_id_of_pk pool payout_address in
      let%bind delegatee_id = pk_id_of_pk pool delegatee_address in
      let%bind _payments_from_delegatee_to_payout_address =
        match%map
          Caqti_async.Pool.use
            (fun db ->
              Sql.User_command.run_payments_by_source_and_receiver db
                ~source_id:delegatee_id ~receiver_id:payment_id )
            pool
        with
        | Ok payments ->
            payments
        | Error err ->
            failwithf
              "Error getting payments from delegatee %s to payment address \
               %s: %s"
              delegatee_address payout_address (Caqti_error.show err) ()
      in
      ...
      in
      let _staking_seed = Epoch_seed.of_string staking_seed_str in
      let next_epoch_ledger_hash =
        Frozen_ledger_hash.of_string next_epoch_ledger_hash_str
      in
      let _next_seed = Epoch_seed.of_string next_seed_str in
      ...
      *)
      ...
      let rec apply_commands (internal_cmds : Sql.Internal_command.t list)
        ...
        | [], [] ->
            ...
            let epoch_ledger_error () = Core_kernel.exit 1 in
      ...
      [%log info] "At genesis, ledger hash"
        ~metadata:[("ledger_hash", json_ledger_hash_of_ledger ledger)] ;
      let%bind _staking_epoch_ledger, _next_epoch_ledger =
...
let () =
  Command.(
    run
      ...
      Command.async ~summary:"Validate block producer obligations"
        (let%map input_file =
         ...
         and archive_uri =
           Param.flag "--archive-uri"
             ...
             Param.(required string)
         and payout_address =
           Param.flag "--payout-address"
             ~doc:
               "PUBKEY Public key of the address receiving delegation rewards"
             Param.(required string)
         in
         main ~input_file ~archive_uri ~payout_address)))
